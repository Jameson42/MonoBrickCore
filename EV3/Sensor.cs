...
namespace MonoBrick.EV3
{
    ...
    public class Sensor : ISensor
    {
        ...
        public SensorType GetSensorType()
        {
            if (!isInitialized)
                Initialize();
            GetTypeAndMode(out SensorType type, out _);
            return type;
        }
        ...
        {
            GetTypeAndMode(out _, out SensorMode mode);
            return mode;
        }
        ...
        protected void GetTypeAndMode(out SensorType type, out SensorMode mode)
        {
            ...
            command.Append(1, VariableScope.Global);
        ...
        protected string GetModeName() => GetModeName(mode);
        ...
        protected void CalcMinMax(uint min, uint max)
        {
        ...
        /// </summary>
        /// <param name="min">Minimum.</param>
        protected void CalcMin(uint min)
        {
            throw new NotImplementedException();
        }
        ...
        /// </summary>
        /// <param name="max">Max.</param>
        protected void CalcMax(uint max)
        {
            throw new NotImplementedException();
        }
        ...
        protected byte SetUp(byte repeat, short reapeatTime, byte[] writeData)
        {
            if (!isInitialized)
                Initialize();
            var command = new Command(4, 0, 201, true);
    ...
    public class TouchSensor : Sensor
    {
        ...
        public override string ReadAsString()
        {
            ...
            {
                s = Convert.ToBoolean(ReadSi()) ? "On" : "Off";
            }
            return s;
        }
    ...
    /// </summary>
    internal class UltrasonicSensor : Sensor
    {
        /// <summary>
    ...
    /// </summary>
    internal class GyroSensor : Sensor
    {
        /// <summary>
    ...
    public static class SensorHelper
    {
        ...
        public static Dictionary<string, Sensor> SensorDictionary
        {
            get
            {
                ...
                {
                    { Color, new ColorSensor(ColorMode.Color) },
                    { ColorAmbient, new ColorSensor(ColorMode.Ambient) },
                    { ColorReflection, new ColorSensor(ColorMode.Reflection) },
                    { ColorReflectionRaw, new ColorSensor(ColorMode.Raw) },
                    { Touch, new TouchSensor(TouchMode.Boolean) },
                    { TouchCount, new TouchSensor(TouchMode.Count) },
                    { GyroAngle, new GyroSensor(GyroMode.Angle) },
                    { GyroAngularVelocity, new GyroSensor(GyroMode.AngularVelocity) },
                    { IrProximity, new IRSensor(IRMode.Proximity) },
                    { IrRemoteMode, new IRSensor(IRMode.Remote) },
                    { IrSeekMode, new IRSensor(IRMode.Seek) },
                    { LightAmbient, new LightSensor(LightMode.Ambient) },
                    { LightReflection, new LightSensor(LightMode.Relection) },
                    { SounddBa, new SoundSensor(SoundMode.SoundDBA) },
                    { SounddB, new SoundSensor(SoundMode.SoundDB) },
                    { TemperatureCelcius, new TemperatureSensor(TemperatureMode.Celcius) },
                    { TemperatureFahrenheit, new TemperatureSensor(TemperatureMode.Fahrenheit) },
                    { UltrasonicCentimeter, new UltrasonicSensor(UltrasonicMode.Centimeter) },
                    { UltrasonicInch, new UltrasonicSensor(UltrasonicMode.Inch) },
                    { UltrasonicListenMode, new UltrasonicSensor(UltrasonicMode.Listen) }
                };
                return dictionary;
            }
        }

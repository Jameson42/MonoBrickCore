...
namespace MonoBrick.EV3
{
    ...
    public class Command : BrickCommand
    {
        ...
        public Command(byte[] data)
        {
            if (data.Length < 4)
            {
                throw new ArgumentException("Invalid EV3 Command");
            }
            ...
            try
            {
                ...
                {
                    SystemCommandType = (SystemCommand)data[3];
                }
                else
                {
                    SystemCommandType = SystemCommand.None;
                }
        ...
        public Command(SystemCommand systemCommand, ushort sequenceNumber, bool reply)
        {
            ...
            CommandType = CommandType.SystemCommand;
        ...
        {
            SystemCommandType = SystemCommand.None;
            CommandType = CommandType.DirectCommand;
            SequenceNumber = sequenceNumber;
            Append(sequenceNumber);
            if (reply)
            {
    ...
    public class Reply : BrickReply
    {
        ...
        /// The type of error
        /// </value>
        internal ErrorType ErrorType => Error.ToErrorType(ErrorCode);
                ...
                if (HasError)
                {
                    if (CommandType == CommandType.SystemReplyWithError)
                    {
                        if (dataArray.Length >= 5)
                        {
                            ...
                            if (Enum.IsDefined(typeof(BrickError), (int)error))
                            {
                                return error;
                            }
                        }
        ...
        public Reply(byte[] data)
        {
            ...
            if (data.Length < 4)
            {
                throw new ArgumentException("Invalid EV3 Reply");
            }
        ...
        /// </summary>
        /// <value>The sequence number.</value>
        public ushort SequenceNumber => (ushort)(0x0000 | dataArray[0] | (dataArray[1] << 2));
        ...
        /// </value>
        public string CommandTypeAsString => BrickCommand.AddSpacesToString(CommandType.ToString());

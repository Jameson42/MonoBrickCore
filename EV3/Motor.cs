...
namespace MonoBrick.EV3
{
    ...
    public class Output
    {
        ...
        public void SetType(MotorPort port, byte type, bool reply = false)
        {
            ...
            command.Append(DaisyChainLayer);
            command.Append(port);
        ...
        public void SetPosition(int position, bool reply = false)
        {
        ...
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void SetStepPower(sbyte power, uint rampUpSteps, uint constantSpeedSteps, uint rampDownSteps, bool brake, bool reply = false)
        {
        ...
        /// <param name="brake">If set to <c>true</c> brake when done.</param>
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void SetTimePower(byte power, uint rampUpTime, uint constantSpeedTime, uint rampDownTime, bool brake, bool reply = false)
        {
        ...
        /// <param name="brake">If set to <c>true</c> brake when done.</param>
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void SetStepSpeed(sbyte speed, uint rampUpSteps, uint constantSpeedSteps, uint rampDownSteps, bool brake, bool reply = false)
        {
        ...
        /// <param name="brake">If set to <c>true</c> brake when done</param>
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void SetTimeSpeed(sbyte speed, uint rampUpTime, uint constantSpeedTime, uint rampDownTime, bool brake, bool reply = false)
        {
        ...
        public void SetStepSync(sbyte speed, short turnRatio, uint steps, bool brake, bool reply = false)
        {
            var command = new Command(0, 0, 209, reply);
        ...
        /// <param name="brake">If set to <c>true</c> brake.</param>
        /// <param name="reply">If set to <c>true</c> reply.</param>
        public void SetTimeSync(sbyte speed, short turnRatio, uint timeInMs, bool brake, bool reply = false)
        {
            var command = new Command(0, 0, 210, reply);
        ...
        /// <param name="port">Motor port to use</param>
        public int GetCount(MotorPort port)
        {
        ...
        public sbyte GetSpeed(MotorPort port)
        {
            ...
            command.Append(port);
            command.Append(0, VariableScope.Global);
            command.Append(4, VariableScope.Global);
            var brickReply = connection.SendAndReceive(command);
    ...
    public class MotorBase
    {
        ...
        public MotorBase()
        {
            DaisyChainLayer = DaisyChainLayer.EV3;
            BitField = OutputBitfield.OutA;
        }
    ...
    public class Motor : MotorBase, IMotor
    {
        ...
        /// </param>
        public void On(sbyte speed, uint degrees, bool brake)
        {
        ...
        /// If set to <c>true</c> the brick will send a reply
        /// </param>
        public void On(sbyte speed, uint degrees, bool brake, bool reply)
        {
            var longDegrees = (ulong)degrees;
            var rampUpDownSteps = (uint)(15 * longDegrees * 100) / 10000;
            var constantsSteps = (uint)(70 * longDegrees * 100) / 10000;
        ...
        /// Set to <c>true</c> if the motor should brake when done
        /// </param>
        public void MoveTo(byte speed, int position, bool brake)
        {
        ...
        /// If set to <c>true</c> the brick will send a reply
        /// </param>
        public void MoveTo(byte speed, int position, bool brake, bool reply)
        {
            var currentPos = GetTachoCount();
            bool moveForward;
            uint diff;
            if (currentPos < position)
            {
                diff = (uint)(position - currentPos);
                moveForward = true;
            }
            else
            {
                diff = (uint)(currentPos - position);
                moveForward = false;
            }
            sbyte motorSpeed;
            if (moveForward)
        ...
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void SpeedProfileStep(sbyte speed, uint rampUpSteps, uint constantSpeedSteps, uint rampDownSteps, bool brake, bool reply = false)
        {
        ...
        /// <param name="brake">If set to <c>true</c> the motor will brake when movement is done.</param>
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void SpeedProfileTime(sbyte speed, uint rampUpTimeMs, uint constantSpeedTimeMs, uint rampDownTimeMs, bool brake, bool reply = false)
        {
        ...
        /// <param name="brake">If set to <c>true</c> the motor will brake when movement is done.</param>
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void PowerProfileStep(sbyte power, uint rampUpSteps, uint constantSpeedSteps, uint rampDownSteps, bool brake, bool reply = false)
        {
        ...
        /// <param name="brake">If set to <c>true</c> the motor will brake when movement is done.</param>
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void PowerProfileTime(byte power, uint rampUpTimeMs, uint constantSpeedTimeMs, uint rampDownTimeMs, bool brake, bool reply = false)
        {
    ...
    public class MotorSync : MotorBase
    {
        ...
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void StepSync(sbyte speed, short turnRatio, uint steps, bool brake, bool reply = false)
        {
        ...
        /// <param name="brake">If set to <c>true</c> motors will brake when done otherwise off.</param>
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void TimeSync(sbyte speed, short turnRatio, uint timeInMs, bool brake, bool reply = false)
        {
        ...
        /// <param name="turnRatio">Turn ratio (-200 to 200).</param>
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void On(sbyte speed, short turnRatio, bool reply)
        {
        ...
        public void On(sbyte speed, short turnRatio, uint degrees)
        {
        ...
        /// <param name="degrees">Degrees to move.</param>
        /// <param name="reply">If set to <c>true</c> reply from brick will be send.</param>
        public void On(sbyte speed, short turnRatio, uint degrees, bool reply)
        {
    ...
    {
        private readonly MotorSync motorSync = new MotorSync();
        ...
        public void Backward(sbyte speed, uint degrees, bool reply = false)
        {
        ...
        public void Forward(sbyte speed, uint degrees, bool reply = false)
        {
        ...
        /// <param name="degrees">Degrees.</param>
        /// <param name="reply">If set to <c>true</c> reply will be send.</param>
        public void SpinLeft(sbyte speed, uint degrees, bool reply = false)
        {
        ...
        /// <param name="degrees">Degrees.</param>
        /// <param name="reply">If set to <c>true</c> reply will be send.</param>
        public void SpinRight(sbyte speed, uint degrees, bool reply = false)
        {
            if (leftPort < rightPort)
            {
                HandleSpinRight(speed, degrees, reply);
        ...
        /// <param name="degrees">Degrees.</param>
        /// <param name="reply">If set to <c>true</c> reply will be send.</param>
        public void TurnRightForward(sbyte speed, sbyte turnPercent, uint degrees, bool reply = false)
        {
            if (leftPort < rightPort)
            {
        ...
        /// <param name="degrees">Degrees.</param>
        /// <param name="reply">If set to <c>true</c> reply will be send.</param>
        public void TurnRightReverse(sbyte speed, sbyte turnPercent, uint degrees, bool reply = false)
        {
            if (leftPort < rightPort)
            {
        ...
        /// <param name="degrees">Degrees.</param>
        /// <param name="reply">If set to <c>true</c> reply will be send.</param>
        public void TurnLeftForward(sbyte speed, sbyte turnPercent, uint degrees, bool reply = false)
        {
            if (leftPort < rightPort)
            {
                HandleLeftForward(speed, turnPercent, degrees, reply);
        ...
        /// <param name="degrees">Degrees.</param>
        /// <param name="reply">If set to <c>true</c> reply.</param>
        public void TurnLeftReverse(sbyte speed, sbyte turnPercent, uint degrees, bool reply = false)
        {
            if (leftPort < rightPort)
            {
                HandleLeftReverse(speed, turnPercent, degrees, reply);
        ...
        private void HandleLeftForward(sbyte speed, sbyte turnPercent, uint degrees, bool reply)
        {
            ...
            if (!ReverseLeft && ReverseRight)
            {
                motorSync.On((sbyte)-speed, (short)(-200 + turnPercent), degrees, reply);
            }
            if (ReverseLeft && !ReverseRight)
            {
                motorSync.On(speed, (short)(-200 + turnPercent), degrees, reply);
            }
            if (ReverseLeft && ReverseRight)
            {
        ...
        private void HandleRightForward(sbyte speed, sbyte turnPercent, uint degrees, bool reply)
        {
            if (!ReverseLeft && !ReverseRight)
            {
            ...
            if (ReverseLeft && ReverseRight)
            {
                motorSync.On((sbyte)-speed, turnPercent, degrees, reply);
            }
        }

        private void HandleLeftReverse(sbyte speed, sbyte turnPercent, uint degrees, bool reply)
        {
            if (!ReverseLeft && !ReverseRight)
            {
                motorSync.On((sbyte)-speed, (short)-turnPercent, degrees, reply);
        ...
        private void HandleRightReverse(sbyte speed, sbyte turnPercent, uint degrees, bool reply)
        {
            if (!ReverseLeft && !ReverseRight)
            {
                motorSync.On((sbyte)-speed, turnPercent, degrees, reply);
            ...
            if (ReverseLeft && ReverseRight)
            {
                motorSync.On(speed, turnPercent, degrees, reply);
            }

        }

        private void HandleSpinRight(sbyte speed, uint degrees, bool reply)
        {
            if (!ReverseLeft && !ReverseRight)
            {
        ...
        private void HandleSpinLeft(sbyte speed, uint degrees, bool reply)
        {
            if (!ReverseLeft && !ReverseRight)
            {
                motorSync.On(speed, -200, degrees, reply);
        ...
        private void Move(sbyte speed, uint degrees, bool reply)
        {
            if (!ReverseLeft && !ReverseRight)
            {
                motorSync.On(speed, 0, degrees, reply);
